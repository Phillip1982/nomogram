...
```{r r_global_options, echo=FALSE, include=FALSE}
library(knitr)
opts_knit$set(root.dir = normalizePath("../"), 
              fig.width=8, fig.height=6, 
               echo=FALSE, include=FALSE,
               warning=FALSE, message=FALSE, 
               cache=TRUE, tidy=TRUE, comment="", tidy.opts=list(width.cutoff=80))
options(scipen = 6)
```
...
create_plot_num <- function(data) {funModeling::plot_num(data, path_out = "~/Dropbox/Nomogram/nomogram/results")} #Export results

create_plot_cross_plot<- function(data) {funModeling::cross_plot(data, input=(colnames(data)), target="Match_Status", path_out = "~/Dropbox/Nomogram/nomogram/results")} #, auto_binning = FALSE, #Export results

create_profiling_num <- function(data) {funModeling::profiling_num(all_data)}
...
colnames(all_data)[colnames(all_data)=="Count_of_Peer_Reviewed_Journal_Articles_Abstracts_Other_than_Published"] <- "Count_of_Peer_Reviewed_Other_than_Published"

colnames(all_data)[colnames(all_data)=="Count_of_Peer_Reviewed_Journal_Articles_Abstracts"] <- "Count_of_Peer_Reviewed_Articles_Abstracts"
...
ggpairs(data=all_data_ggally, progress=TRUE, columns = c("Match_Status" ,  "Medical_Education_Interrupted", "Alpha_Omega_Alpha", "USMLE_Step_1_Score"))
...
Hmisc::label(all_data$Medical_Education_Interrupted) <- 'Medical School Interrupted'
...
Hmisc::label(all_data$Count_of_Peer_Reviewed_Articles_Abstracts) <- 'Count of Journal Articles'
Hmisc::label(all_data$Count_of_Peer_Reviewed_Other_than_Published) <-'Count of Research Not Published'
...
s <- summary(Match_Status ~ cut2(Age, 30:30) + Gender + Alpha_Omega_Alpha + cut2(USMLE_Step_1_Score, 245:245) + Couples_Match + Medical_Education_Interrupted + US_or_Canadian_Applicant + Military_Service_Obligation + Count_of_Oral_Presentation + cut2(Count_of_Peer_Reviewed_Book_Chapter, 0:3) + cut2(Count_of_Poster_Presentation, 0:3) + white_non_white + cut2(Count_of_Peer_Reviewed_Articles_Abstracts, 0:3) + cut2(Count_of_Peer_Reviewed_Other_than_Published, 0:3), data = all_data)
s
```
...
stargazer::stargazer(all_data[1:4, ], output='latex', summary=FALSE, rownames=FALSE, header = TRUE)
```
...
```{r rpart EDA}
t.model <- rpart((Match_Status~.), data = train)
t.model$variable.importance
```

The simple tree was plotted below. 
#MAYBE BIN ALL NUMERIC VARIABLES TO USE IN R PLOT
...
shapiro.test(all_data$Count_of_Peer_Reviewed_Articles_Abstracts)
...
kitchen.sink <- lrm(Match_Status ~ white_non_white +  rcs(Age, 5) + Gender +  Couples_Match + US_or_Canadian_Applicant +  Medical_Education_Interrupted + Alpha_Omega_Alpha +  Military_Service_Obligation + rcs(USMLE_Step_1_Score, 4) + rcs(Count_of_Poster_Presentation,3) +  Count_of_Oral_Presentation + Count_of_Peer_Reviewed_Articles_Abstracts + Count_of_Peer_Reviewed_Book_Chapter + Count_of_Peer_Reviewed_Other_than_Published + Count_of_Peer_Reviewed_Online_Publication + Visa_Sponsorship_Needed + Medical_Degree, data = train, x = T, y = T)
...
```{r, echo=TRUE}
lm.fit2 <- lm(Match_Status ~ (white_non_white + Age + Gender +  Couples_Match + US_or_Canadian_Applicant +  Medical_Education_Interrupted + Alpha_Omega_Alpha +  Military_Service_Obligation + USMLE_Step_1_Score + Count_of_Poster_Presentation +  Count_of_Oral_Presentation + Count_of_Peer_Reviewed_Articles_Abstracts + Count_of_Peer_Reviewed_Book_Chapter + Count_of_Peer_Reviewed_Other_than_Published + Count_of_Peer_Reviewed_Online_Publication + Visa_Sponsorship_Needed + Medical_Degree)^2, data = all_data)
```
...
set.seed(0)
train_corrleation <- train %>% select(Age, USMLE_Step_1_Score, Count_of_Poster_Presentation, Count_of_Oral_Presentation, Count_of_Peer_Reviewed_Articles_Abstracts, Count_of_Peer_Reviewed_Book_Chapter, Count_of_Peer_Reviewed_Other_than_Published, Count_of_Peer_Reviewed_Online_Publication) %>%
...
`r sample$Medical_Education_Interrupted[1]` 
...
`r sample$Count_of_Peer_Reviewed_Articles_Abstracts[1]`
